...
\`viewr()\` profiles every column in R and renders a virtualized grid
...
- options:
  ...
  A list created by \[viewr_options()\].
...
standalone HTML file via \[save_viewr()\].
...
if (FALSE) { # \dontrun{
viewr(mtcars)
viewr(iris, options = viewr_options(theme = "dark", hidden_columns = "Species"))
} # }
```
